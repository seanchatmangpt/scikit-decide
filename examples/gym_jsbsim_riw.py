...
from autofde_lab import Domain, TransitionOutcome, Value
...
class D(
    GymDomainHashable, GymDiscreteActionDomain, GymWidthDomain, DeterministicInitialized
):
    pass
...
class GymRIWDomain(D):
    ...
    def _state_step(
        ...
        self._cumulated_dist_to_line += math.exp(
            -math.fabs(self._gym_env.sim.get_property_value(prp.shortest_dist))
        )
    ...
    def reset_reward_features(self):
        ...
        self._feature_increments.append(
            [
                [[0] for f in range(self._continuous_feature_fidelity)]
        ...
        if reward > 0:
            for f in range(self._continuous_feature_fidelity):
                i = bisect.bisect_left(
                    self._feature_increments[0][depth][f], reward - ref_val
                )
                features.append(i)
            ...
            for f in range(self._continuous_feature_fidelity):
                i = bisect.bisect_left(
                    self._feature_increments[1][depth][f], ref_val - reward
                )
                features.append(-i)
    ...
    def bee_distance_features(self, state):
        ...
        for feature in state._context[2:]:
            ...
            if feature > 0:
                for f in range(self._continuous_feature_fidelity):
                    i = bisect.bisect_left(
                        self._feature_increments[2 * index][depth][f], feature - ref_val
                    )
                    cf.append(i)
                    if i >= len(self._feature_increments[2 * index][depth][f]):
                        self._feature_increments[2 * index][depth][f].append(
                            feature - ref_val
                        )
                    if i > 0:
                        ref_val = (
                            ref_val
                            + self._feature_increments[2 * index][depth][f][i - 1]
                        )
            else:
                for f in range(self._continuous_feature_fidelity):
                    i = bisect.bisect_left(
                        self._feature_increments[2 * index + 1][depth][f],
                        ref_val - feature,
                    )
                    cf.append(-i)
                    if i >= len(self._feature_increments[2 * index + 1][depth][f]):
                        self._feature_increments[2 * index + 1][depth][f].append(
                            ref_val - feature
                        )
                    if i > 0:
                        ref_val = (
                            ref_val
                            - self._feature_increments[2 * index + 1][depth][f][i - 1]
                        )
            features += cf
            index += 1
        return features
