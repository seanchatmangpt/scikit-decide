...
from autofde_lab import Domain, Solver, Space
from autofde_lab.builders.domain import Sequential, SingleAgent, UnrestrictedActions
...
class _BaseOpenEvolve(Solver, Policies, Restorable):
    ...
    def _sample_action(
        ...
        if self._unrestricted_actions:
            ...
            if domain is None:
                raise ValueError(
                    "The rollout `domain` cannot be None when dealing with restricted actions."
                )
            applicable_actions = autocast(
                domain.get_applicable_actions, domain, self.T_domain
            )()
            return self._convert_action_from_planner(
                self._evolved_planner.sample_action(
                    self._convert_obs_for_planner(observation),
                    self._convert_applicable_actions_for_planner(applicable_actions),
                )
            )
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        evaluator_enforce_using_public_api: if True, the evaluator domains will be wrapped in a proxy that
            ...
            The proxy class still derives from the relevant characteristics found in autofde_lab.builder.domain so that
