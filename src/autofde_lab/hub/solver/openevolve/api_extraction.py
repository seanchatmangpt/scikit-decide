...
import autofde_lab
import autofde_lab.builders.domain as domain_builders
import autofde_lab.hub as hub_module
from autofde_lab import Domain, Space
from autofde_lab.builders.domain import SingleAgent
...
def _extract_domain_base_types(
    ...
    - T_memory -> autofde_lab.builders.domain.memory
    - T_agent -> autofde_lab.builders.domain.agent
    - ...

    # Parameters
    domain_cls
    skip_predefined: if True, skip types that are defined (and unchanged) by characteristic mixins
        (T_memory -> autofde_lab.builders.domain.memory, T_agent -> autofde_lab.builders.domain.agent, ...)
...
def _is_module_to_document(
    ...
    return (
        (
            ...
            and module_name.startswith(autofde_lab.__name__)
...
def is_class_to_document(
    ...
    - from autofde_lab core: always taken
...
class ApiExtractionParams:
    ...
    include_hub_api: if `recursive`, extract api for classes defined in submodules from `autofde_lab.hub`
