...
@pytest.fixture(
    params=[
        ...
        ("state", 123, 1.23),
        ["state", 123, 1.23],
        {"state_A": "state", "state_B": 123, "state_C": 1.23},
    ]
)
def state(request):
...
@pytest.fixture(
    params=[
        ...
        ("action", 321, 3.21),
        ["action", 321, 3.21],
        {"action_A": "action", "action_B": 321, "action_C": 3.21},
    ]
)
def action(request):
    return request.param
...
# SAMPLE FUNCTIONS
def func_in(
    memory: D.T_memory[D.T_state], action: D.T_agent[D.T_concurrency[D.T_event]]
):
