...
def test_integrated_openevolve_maze(
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        with IntegratedOpenEvolve(**solver_kwargs) as solver:
            ...
            assert ("nb_reached_goals" in init_res.metrics) == issubclass(
                domain_cls, Goals
            )
            ...
            if issubclass(domain_cls, FullyObservable):
                ...
                if evaluator_enforce_using_public_api:
                    ...
                    assert (
                        "Access to private member" in private_api_res.artifacts["error"]
                    )
                else:
        ...
        # remove file handlers added by openevolve in temporary directory (make tests crash on windows)
        root_logger = logging.getLogger()
        while root_logger.handlers:
            h = root_logger.handlers.pop()
