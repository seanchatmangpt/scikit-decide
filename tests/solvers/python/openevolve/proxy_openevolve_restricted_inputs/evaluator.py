...
class EvolvedSolver(Solver, Policies):
    ...
    def __init__(self, domain_factory: Callable[[], Maze], program_path: str):
        ...
        self._planner = evolved_planner_cls(
            goal=domain._hub_maze._goal, maze=domain._hub_maze._maze
        )
    ...
    def _sample_action(
        ...
        action_outside_hub = self._planner.sample_action(
            observation, [a.name for a in applicable_actions.get_elements()]
        )
