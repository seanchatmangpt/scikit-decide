...
def get_conda_packages(run_lambda):
    if get_platform() == "win32":
        ...
        grep_cmd = r'{} /R "torch numpy cudatoolkit soumith mkl magma mypy stable-baselines3 scikit-decide"'.format(
            findstr_cmd
        )
    else:
        grep_cmd = r'grep "torch\|numpy\|cudatoolkit\|soumith\|mkl\|magma\|mypy\|stable-baselines3\|scikit-decide"'
...
def get_pip_packages(run_lambda):
    ...
    def run_with_pip(pip):
        if get_platform() == "win32":
            ...
            findstr_cmd = os.path.join(system_root, "System32", "findstr")
            grep_cmd = (
                r'{} /R "numpy torch mypy stable-baselines3 scikit-decide"'.format(
                    findstr_cmd
                )
            )
        else:
            grep_cmd = r'grep "torch\|numpy\|mypy\|stable-baselines3\|scikit-decide"'
